...
#if defined   (__CC_ARM) /*!< ARM Compiler */		//ARM编译器
__align(4) 
...
#elif defined ( __ICCARM__ ) /*!< IAR Compiler *///其他编译器
...
__IO ETH_DMADESCTypeDef  *DMATxDescToSet;		//全局的DMA发送描述符
...
void ETH_PTPTime_GetTime(struct ptptime_t * timestamp) 
{
	...
	i = sys_arch_protect();	//进入系统临界区，未使用系统，应该去掉
  timestamp->tv_nsec = ETH_GetPTPRegister(ETH_PTPTSLR);		//取第1次硬件时间T1
  timestamp->tv_sec  = ETH_GetPTPRegister(ETH_PTPTSHR);
	
	time.tv_nsec 	= ETH_GetPTPRegister(ETH_PTPTSLR);				//接着取第2次硬件时间T2
	time.tv_sec 	= ETH_GetPTPRegister(ETH_PTPTSHR);
	sys_arch_unprotect(i);	//出系统临界区
	
	if((time.tv_nsec < timestamp->tv_nsec))			//如果T2<T1(ns部分)
	{
		if(time.tv_sec == timestamp->tv_sec)			//且T2=T1(s部分)，（说明s刚好进位）
			timestamp->tv_sec = time.tv_sec - 1;		//返回的秒=T2-1
	}
	timestamp->tv_nsec = ETH_PTPSubSecond2NanoSecond(timestamp->tv_nsec);//亚秒转换成ns单位
}
...
void ETH_PHYOUTPUT50M(uint16_t PHYAddress)
{
}
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  _eth_delay_(PHY_RESET_DELAY);
	...
	ETH_PHYOUTPUT50M(PHYAddress);//配置PHY输出50MHz 网卡输出50M
...
{   
  ...
  if((DMATxDescToSet->Status & ETH_DMATxDesc_OWN) != (u32)RESET)
		{  
			return ETH_ERROR;				//检查当前描述符,是否属于ETHERNET DMA(设置的时候)/CPU(复位的时候)
		}
  DMATxDesc = DMATxDescToSet; //要传输的DMA描述符的地址
  if (FrameLength > ETH_TX_BUF_SIZE)	//判断当前发送的数据是否超过1524以太网包最大字节
  {
  ...
  if (buf_count ==1) //只有1帧
  {
    DMATxDesc->Status |= ETH_DMATxDesc_FS | ETH_DMATxDesc_LS; 					//描述符缓冲区的状态-（第一帧，也是最后一帧）
    DMATxDesc->ControlBufferSize = (FrameLength & ETH_DMATxDesc_TBS1); 	//编程帧长度
    DMATxDesc->Status |= ETH_DMATxDesc_OWN;															//设置Tx描述符的OWN位,buffer重归ETH DMA
    DMATxDesc= (ETH_DMADESCTypeDef *)(DMATxDesc->Buffer2NextDescAddr); 	//更新全局Tx描述符为下一个Tx描述符
  }
  else
  {
    for (i=0; i< buf_count; i++)
    {
      /* Clear FIRST and LAST segment bits */
			//如果数据大于1524字节,将被分成几部分
      DMATxDesc->Status &= ~(ETH_DMATxDesc_FS | ETH_DMATxDesc_LS);
      
      if (i==0)  //第一截
      {
        /* Setting the first segment bit */
        DMATxDesc->Status |= ETH_DMATxDesc_FS; //标记数据包的第一截
      }

      /* Program size */
      DMATxDesc->ControlBufferSize = (ETH_TX_BUF_SIZE & ETH_DMATxDesc_TBS1);//第一截的帧长度
      
      if (i== (buf_count-1)) //最后一截
      {
      ...
      DMATxDesc->Status |= ETH_DMATxDesc_OWN; //设置状态标记

      DMATxDesc = (ETH_DMADESCTypeDef *)(DMATxDesc->Buffer2NextDescAddr); //指向下一部分
    }
  }
  
  DMATxDescToSet = DMATxDesc; //将地址传递给DMATxDescToSet
...
/*
  * @brief  Prepares DMA Tx descriptors to transmit an ethernet frame
  * @param  FrameLength : length of the frame to send
	*	@param  TimeStamp : transmit an ethernet frame time Stamp
  * @retval error status
  */
uint32_t ETH_Prepare_Transmit_Descriptors_TimeStamp(u16 FrameLength, ETH_TimeStamp *TimeStamp)
{
  ...
  if ((DMATxDescToSet->Status & ETH_DMATxDesc_OWN) != (u32) RESET)//检查当前描述符,是否属于ETHERNET DMA(设置的时候)/CPU(复位的时候)
		{  
			printf("ERR:OWN when Transmit_Descriptors");
			return ETH_ERROR;		//错误,OWN位被设置了 
		}
	DMATxDesc = DMATxDescToSet;		
	DMATxDesc->Status |= ETH_DMATxDesc_TTSE;		//在初始化网络发送描述符的时候,没有使能发送时间戳标记(所以在这个地方使能)
	DMATxDesc->TimeStampHigh = 0;																				//描述符的时间戳高位清0
	DMATxDesc->TimeStampLow = 0;																				//描述符的时间戳低位清0
	DMATxDesc->Status |= ETH_DMATxDesc_FS | ETH_DMATxDesc_LS; 					//描述符帧首位和末位标识符同时置1
	DMATxDesc->ControlBufferSize = (FrameLength& ETH_DMATxDesc_TBS1);		//描述符帧长度,且只取发送缓存区1的值
	DMATxDesc->Status |= ETH_DMATxDesc_OWN; 														//设置Tx描述符的OWN位,描述符所有权重归ETH DMA
	
  if ((ETH->DMASR & ETH_DMASR_TBUS) != (u32) RESET) 	//当Tx Buffer不可用位(TBUS)被设置的时候,重置它.恢复传输
		{
			ETH->DMASR = ETH_DMASR_TBUS;		//指示发送不可用状态 
	...
	while (!(DMATxDesc->Status & ETH_DMATxDesc_TTSS) && (timeout < PHY_READ_TO));//等待时间戳产生标记(表示时间戳已经产生)
	if (timeout >= PHY_READ_TO)		//超时，错误
		{
			DMATxDesc = (ETH_DMADESCTypeDef *)(DMATxDesc->Buffer2NextDescAddr);	//还没有描述完，换下一个描述符继续	
			DMATxDescToSet = DMATxDesc;											//下一次buffer发送的描述符地址
			return ETH_ERROR;
		}
	TimeStamp->TimeStampHigh = DMATxDesc->TimeStampHigh;															//返回时间戳
	TimeStamp->TimeStampLow  = ETH_PTPSubSecond2NanoSecond(DMATxDesc->TimeStampLow);	//返回时间戳
	//	if(TimeStamp->TimeStampHigh != timestamp.tv_sec) //判断DMA的时间和当前时间是否偏差(检测DMA时间戳是否出错)
	//	{
	//		TimeStamp->TimeStampHigh = timestamp.tv_sec;
	//	}
	DMATxDesc->TimeStampHigh = 0;										//描述符的时间戳高位清0
	DMATxDesc->TimeStampLow = 0;										//描述符的时间戳低位清0
	DMATxDesc->Status &= ~ETH_DMATxDesc_TTSS; 			//清除时间戳产生标记
	//printf("	Buffer1Addr:%x	Buffer2Addr:%x\n",DMATxDesc->Buffer1Addr,DMATxDesc->Buffer2NextDescAddr);	
	DMATxDesc = (ETH_DMADESCTypeDef *)(DMATxDesc->Buffer2NextDescAddr);	//还没有描述完，换下一个描述符继续	
	DMATxDescToSet = DMATxDesc;											//下一次buffer发送的描述符地址
	return ETH_SUCCESS;   
}
                               ...
                               uint8_t* TxBuff, uint32_t TxBuffCount)
{
  ...
  DMATxDescToSet	  = DMATxDescTab; 		//DMATxDescToSet指向发送DMA描述表的首地址
  DMAPTPTxDescToSet = DMAPTPTxDescTab;  //DMAPTPTxDescToSet指向PTP的发送DMA描述表的首地址
  for(i=0; i < TxBuffCount; i++)				//填充DMA描述表
  {
    DMATxDesc = DMATxDescTab+i;					//第i个DMA描述表（2纬数组）
    DMATxDesc->Status = ETH_DMATxDesc_TCH | ETH_DMATxDesc_TTSE; //设置描述符链表开启时间戳  
    DMATxDesc->Buffer1Addr =(uint32_t)(&TxBuff[i*ETH_MAX_PACKET_SIZE]);	//设置Buffer1地址
    if(i < (TxBuffCount-1))
    {
      DMATxDesc->Buffer2NextDescAddr = (uint32_t)(DMATxDescTab+i+1);//下一个描述符地址
    }
    else
    {
      DMATxDesc->Buffer2NextDescAddr = (uint32_t) DMATxDescTab;  		//最后一个描述符指回头部
    }
    (&DMAPTPTxDescTab[i])->Buffer1Addr = DMATxDesc->Buffer1Addr;									//PTP描述符包含了普通描述符表
    (&DMAPTPTxDescTab[i])->Buffer2NextDescAddr = DMATxDesc->Buffer2NextDescAddr;	//PTP描述符包含了普通描述符表
  }
  (&DMAPTPTxDescTab[i-1])->Status = (uint32_t) DMAPTPTxDescTab;			//保存PTP描述符的状态
  ETH->DMATDLAR = (uint32_t) DMATxDescTab;		//将描述符地址写进寄存器(发送列表起始处)
}
...
void ETH_DMATxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff, uint32_t TxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADESCTypeDef *DMATxDesc;
  DMATxDescToSet = DMATxDescTab; 			//全局DMA描述符指向形参
  for(i=0; i < TxBuffCount; i++)			//填充描述符表
  {
    DMATxDesc = DMATxDescTab + i;
    DMATxDesc->Status = ETH_DMATxDesc_TCH;														//填充描述符的状态位_链表结构
    DMATxDesc->Buffer1Addr = (uint32_t)(&TxBuff[i*ETH_TX_BUF_SIZE]); 	//填充描述符的缓存区指针
    if(i < (TxBuffCount-1))	
    {
      DMATxDesc->Buffer2NextDescAddr = (uint32_t)(DMATxDescTab+i+1);	//填充描述符的下一链
    }
    else
    {
      DMATxDesc->Buffer2NextDescAddr = (uint32_t) DMATxDescTab;  			//如果是最后一个，链向描述符表头
    }
  }
  ETH->DMATDLAR = (uint32_t) DMATxDescTab;	//描述符表的地址写入寄存器
}
...
uint32_t ETH_GetRxPktSize(void)
{
     ...
     ((DMARxDescToGet->Status & ETH_DMARxDesc_FS)  != (uint32_t)RESET))
  {
    ...
    frameLength = ETH_GetDMARxDescFrameLength(DMARxDescToGet); //获取数据缓存描述符并返回整个包的长度
  }
...
void ETH_EnablePTPTimeStampAddend(void)
{
  ...
  ETH->PTPTSCR |= ETH_PTPTSCR_TSARU;    	//时间戳加数寄存器更新
}
...
{
	while((ETH->PTPTSCR & 0x00000004) == 0)
