...
//					dmsg.ethernet_mode = rtOpts.transport;
					dmsg.portState=G_ptpClock.portState-1;
					if(dmsg.portState>8)
						dmsg.portState=PTP_FAULTY;
					dmsg.timeSource = G_ptpClock.timePropertiesDS.timeSource;
					if(dmsg.portState==PTP_MASTER)
						{
							dmsg.offsetFromMaster_nanoseconds =	G_ptpClock.offsetFromMaster.nanoseconds;
							dmsg.offsetFromMaster_seconds 		=	G_ptpClock.offsetFromMaster.seconds;
							dmsg.meanPathDelay_nanoseconds		= G_ptpClock.meanPathDelay.nanoseconds;
							dmsg.meanPathDelay_seconds			  = G_ptpClock.meanPathDelay.seconds;
						}
					else if(dmsg.portState==PTP_SLAVE)
						{
					...
					myprintf("offset=%d.%d\n",dmsg.offsetFromMaster_seconds,dmsg.offsetFromMaster_nanoseconds);
					myprintf("delay=%d.%d\n",dmsg.meanPathDelay_seconds,dmsg.meanPathDelay_nanoseconds);	
