...
typedef struct {
	...
	Boolean currentUtcOffsetValid;	//currentUtcOffset已知时，应该有效
	Boolean leap59;
	Boolean leap61;
...
//	Octet* ifaceName;
	...
	Boolean	noResetClock; // don't step the clock if offset > 1s				  一步调整到位或者逐步调整
	...
	Integer16 	s;
	TimeInternal inboundLatency, outboundLatency, ofmShift;	//进入系统的延迟补偿，离开系统的延迟补偿，偏移
...
typedef struct timeInfo
{
	...
	unsigned int  pdelay_req_send_time_s; 			//P2P的4个时间
	unsigned int  pdelay_req_send_time_ns; 
	unsigned int  pdelay_req_receive_time_s; 		//P2P的4个时间
	unsigned int  pdelay_req_receive_time_ns; 
	unsigned int  pdelay_resp_send_time_s; 			//P2P的4个时间
	unsigned int  pdelay_resp_send_time_ns; 
	unsigned int  pdelay_resp_receive_time_s;		//P2P的4个时间
	unsigned int  pdelay_resp_receive_time_ns;
	int   offsetFromMaster_s;										//Tms
	int   offsetFromMaster_ns;
	int   meanPathDelay_s;											//网络延迟
	int   meanPathDelay_ns;
	int   peerMeanPathDelay_s;									//P2P链路延迟
